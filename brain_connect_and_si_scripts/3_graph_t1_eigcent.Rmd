---
title: "graph_t1_eigenvect_centrality"
author: "Jackie"
date: "10/21/2020"
...
# reading data
```{r message=FALSE}
eig_cent_fp <- "~/Box/Mooddata_Coordinating/1_Lab_Coordinating/Users/JackieSchwartz/PHIND_RS_and_Suicide/Octo2020Analysis/results/eig_cent.xlsx"
eig_cent_th.1 <-
  read_excel(eig_cent_fp, sheet = "eig_cent_.1")
eig_cent_th.12 <-
  read_excel(eig_cent_fp, sheet = "eig_cent_.12")
eig_cent_th.14 <-
  read_excel(eig_cent_fp, sheet = "eig_cent_.14")
eig_cent_th.16 <-
  read_excel(eig_cent_fp, sheet = "eig_cent_.16")
eig_cent_th.18 <-
  read_excel(eig_cent_fp, sheet = "eig_cent_.18")
eig_cent_th.2 <-
  read_excel(eig_cent_fp, sheet = "eig_cent_.2")
```
...
eig_cent_th.1 <-
  ...
  mutate(ELS_ID = 
  str_remove(ELS_ID, 
             "_ses-T1_task-rest_space-MNIPediatricAsym_cohort-4_res-2-BrainnetomeTimeseries"
             ),
  ELS_ID =
    str_remove(ELS_ID,
               "sub-")
...
## renaming variables
```{r}
bg_rois_fp <- "~/Box/Mooddata_Coordinating/1_Lab_Coordinating/Users/JackieSchwartz/PHIND_RS_and_Suicide/Octo2020Analysis/bn_timeser_102020/BrainRegions_Brainnetome_250_wBG_rc.xlsx"
...
eig_cent_th.1_long <- 
  eig_cent_th.1 %>%
  gather(oldroi_name,eig_cent_value, A8mL:lPFthaR) %>%
  mutate(oldroi_name = factor(oldroi_name))

eig_cent_th.1_long_rename <-
  eig_cent_th.1_long %>%
  mutate(
    newroi_name =
  do.call(fct_recode,
...
eig_cent_th.1_wide <-
  eig_cent_th.1_long_rename %>%
  dplyr::select(ELS_ID, newroi_name, eig_cent_value) %>%
  spread(newroi_name, eig_cent_value)
...
```{r}
eig_cent_all_thr <-
  left_join(eig_cent_th.1_wide,
            eig_cent_th.12_wide,
            by = "ELS_ID")
eig_cent_all_thr <-
  left_join(eig_cent_all_thr,
            eig_cent_th.14_wide,
            by = "ELS_ID")
eig_cent_all_thr <-
  left_join(eig_cent_all_thr,
...
```{r long then wide}
# gather
eig_cent_all_thr_long <- 
  eig_cent_all_thr %>%
  group_by(ELS_ID) %>%
  gather(roi, eig_cent_value, 
         lMFG_L_27_.10:vmPu_R_230_.20) %>%
  separate(col = roi, c("roi", "thresh"), "[.]")
# spread
eig_cent_all_thr_wide <-
  eig_cent_all_thr_long %>%
  spread(thresh, eig_cent_value)  %>%
  mutate(roi = 
...
# degree by threshold
dist_eig_cent_by_thresh <- 
  eig_cent_all_thr_long %>%
  mutate(
    thresh = as.factor(thresh)
  ) %>%
  ggplot(.,
         aes(x=roi, y=eig_cent_value)
         ) +
...
amyg_auc_wide <-
  amyg_auc %>%
  dplyr::select(ELS_ID, roi, amyg_S_AUCg) %>%
...
cg_auc_wide <-
  cg_auc %>%
  dplyr::select(ELS_ID, roi, cg_S_AUCg) %>%
...
fug_auc_wide <-
  fug_auc %>%
  dplyr::select(ELS_ID, roi, fug_S_AUCg) %>%
...
hipp_auc_wide <-
  hipp_auc %>%
  dplyr::select(ELS_ID, roi, hipp_S_AUCg) %>%
...
ifg_auc_wide <-
  ifg_auc %>%
  dplyr::select(ELS_ID, roi, ifg_S_AUCg) %>%
...
ins_auc_wide <-
  ins_auc %>%
  dplyr::select(ELS_ID, roi, ins_S_AUCg) %>%
...
ipl_auc_wide <-
  ipl_auc %>%
  dplyr::select(ELS_ID, roi, ipl_S_AUCg) %>%
...
itg_auc_wide <-
  itg_auc %>%
  dplyr::select(ELS_ID, roi, itg_S_AUCg) %>%
...
locc_auc_wide <-
  locc_auc %>%
  dplyr::select(ELS_ID, roi, locc_S_AUCg) %>%
...
mfg_auc_wide <-
  mfg_auc %>%
  dplyr::select(ELS_ID, roi, mfg_S_AUCg) %>%
...
mtg_auc_wide <-
  mtg_auc %>%
  dplyr::select(ELS_ID, roi, mtg_S_AUCg) %>%
...
mvocc_auc_wide <-
  mvocc_auc %>%
  dplyr::select(ELS_ID, roi, mvocc_S_AUCg) %>%
...
org_auc_wide <-
  org_auc %>%
  dplyr::select(ELS_ID, roi, org_S_AUCg) %>%
...
pcl_auc_wide <-
  pcl_auc %>%
  dplyr::select(ELS_ID, roi, pcl_S_AUCg) %>%
...
pcun_auc_wide <-
  pcun_auc %>%
  dplyr::select(ELS_ID, roi, pcun_S_AUCg) %>%
...
phg_auc_wide <-
  phg_auc %>%
  dplyr::select(ELS_ID, roi, phg_S_AUCg) %>%
...
pog_auc_wide <-
  pog_auc %>%
  dplyr::select(ELS_ID, roi, pog_S_AUCg) %>%
...
prg_auc_wide <-
  prg_auc %>%
  dplyr::select(ELS_ID, roi, prg_S_AUCg) %>%
...
psts_auc_wide <-
  psts_auc %>%
  dplyr::select(ELS_ID, roi, psts_S_AUCg) %>%
...
sfg_auc_wide <-
  sfg_auc %>%
  dplyr::select(ELS_ID, roi, sfg_S_AUCg) %>%
...
spl_auc_wide <-
  spl_auc %>%
  dplyr::select(ELS_ID, roi, spl_S_AUCg) %>%
...
stg_auc_wide <-
  stg_auc %>%
  dplyr::select(ELS_ID, roi, stg_S_AUCg) %>%
...
cttha_auc_wide <-
  cttha_auc %>%
  dplyr::select(ELS_ID, roi, cttha_S_AUCg) %>%
...
dca_auc_wide <-
  dca_auc %>%
  dplyr::select(ELS_ID, roi, dca_S_AUCg) %>%
...
dlpu_auc_wide <-
  dlpu_auc %>%
  dplyr::select(ELS_ID, roi, dlpu_S_AUCg) %>%
...
gpe_auc_wide <-
  gpe_auc %>%
  dplyr::select(ELS_ID, roi, gpe_S_AUCg) %>%
...
gpi_auc_wide <-
  gpi_auc %>%
  dplyr::select(ELS_ID, roi, gpi_S_AUCg) %>%
...
lpftha_auc_wide <-
  lpftha_auc %>%
  dplyr::select(ELS_ID, roi, lpftha_S_AUCg) %>%
...
mpftha_auc_wide <-
  mpftha_auc %>%
  dplyr::select(ELS_ID, roi, mpftha_S_AUCg) %>%
...
mpmtha_auc_wide <-
  mpmtha_auc %>%
  dplyr::select(ELS_ID, roi, mpmtha_S_AUCg) %>%
...
nac_auc_wide <-
  nac_auc %>%
  dplyr::select(ELS_ID, roi, nac_S_AUCg) %>%
...
otha_auc_wide <-
  otha_auc %>%
  dplyr::select(ELS_ID, roi, otha_S_AUCg) %>%
...
pptha_auc_wide <-
  pptha_auc %>%
  dplyr::select(ELS_ID, roi, pptha_S_AUCg) %>%
...
rttha_auc_wide <-
  rttha_auc %>%
  dplyr::select(ELS_ID, roi, rttha_S_AUCg) %>%
...
stha_auc_wide <-
  stha_auc %>%
  dplyr::select(ELS_ID, roi, stha_S_AUCg) %>%
...
vca_auc_wide <-
  vca_auc %>%
  dplyr::select(ELS_ID, roi, vca_S_AUCg) %>%
...
vmpu_auc_wide <-
  vmpu_auc %>%
  dplyr::select(ELS_ID, roi, vmpu_S_AUCg) %>%
...
sn_auc_wide <-
  sn_auc %>%
  dplyr::select(ELS_ID, roi, sn_S_AUCg) %>%
...
### write csv
```{r}
eig_cent_aucs_select_fp <-"~/Box/Mooddata_Coordinating/1_Lab_Coordinating/Users/JackieSchwartz/PHIND_RS_and_Suicide/Octo2020Analysis/eig_cent_aucs.csv"
write_csv(eig_cent_aucs_select,eig_cent_aucs_select_fp)
```
