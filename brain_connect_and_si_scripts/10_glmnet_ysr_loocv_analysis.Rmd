...
ysr_siq_graph_select <-
  ...
  # converting race/ethinicity to factor and binarizing for analysis
    mutate(KSADS_Child_Race_by_P.T1 = factor(KSADS_Child_Race_by_P.T1),
         ...
         # converting sex and race to factor
  ...
  dplyr::select(-KSADS_Child_Race_by_P.T1, -siq_total_log, -siq_total.T3, -Age_S1.T3, -Tanner_Average.T3, -T1_KSADS_C_or_P_Thoughts, -T1_KSADS_C_or_P_bx, -T1_YSR_old_Withdrawn_Total_Score, -T1_YSR_old_Somatic_Complaints_Total_Score, -T1_YSR_old_AnxiousDepressed_Total_Score, -T3_YSR_New_AnxiousDepressed_Total_Score, -T3_YSR_New_WithdrawnDepressed_Total_Score, -T3_YSR_New_Somatic_Complaints_Total_Score) %>%
  ...
  mutate_at(vars(-ELS_ID, -sex_child, -KSADS_Child_Race_by_P.T1_bin), z_score) %>%
  ...
  dplyr::select(-T3_YSR_New_Internalizing_Problems_Total_Score, -ELS_ID, -meds_baseline)
...
ysr_predictors_scaled <- data.matrix(ysr_siq_graph_select[2:1262]) # columns 2 - 1261 are predictors
...
ysr_lasso_results <- ysr_loocv_lasso_coef %>%
  ...
  theme_blank() +
  scale_fill_manual(values = c("Clinical Characteristic" = "cornflowerblue",
                               "Demographic" = "cadetblue",
                               "Frontal Gyrus" = "violet",
                               "Temporal Gyrus" = "gold1"))
  #scale_fill_brewer(palette="Set2")

ggsave("ysr_lasso_results.jpg", ysr_lasso_results, width = 9, height = 5)
```
