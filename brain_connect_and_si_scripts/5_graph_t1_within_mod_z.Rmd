...
# reading data
```{r message=FALSE}
wm_z_fp <- "~/Box/Mooddata_Coordinating/1_Lab_Coordinating/Users/JackieSchwartz/PHIND_RS_and_Suicide/Octo2020Analysis/results/within_mod_z_score.xlsx"
wm_z_th.1 <-
  read_excel(wm_z_fp, sheet = "wm_z_.1")
wm_z_th.12 <-
  read_excel(wm_z_fp, sheet = "wm_z_.12")
wm_z_th.14 <-
  read_excel(wm_z_fp, sheet = "wm_z_.14")
wm_z_th.16 <-
  read_excel(wm_z_fp, sheet = "wm_z_.16")
wm_z_th.18 <-
  read_excel(wm_z_fp, sheet = "wm_z_.18")
wm_z_th.2 <-
  read_excel(wm_z_fp, sheet = "wm_z_.2")
```
...
wm_z_th.1_long <- 
  wm_z_th.1 %>%
  gather(oldroi_name,wm_z_value, A8mL:lPFthaR) %>%
  mutate(oldroi_name = factor(oldroi_name))

wm_z_th.1_long_rename <-
  wm_z_th.1_long %>%
  mutate(
    newroi_name =
  do.call(fct_recode,
...
wm_z_th.1_wide <-
  wm_z_th.1_long_rename %>%
  dplyr::select(ELS_ID, newroi_name, wm_z_value) %>%
  spread(newroi_name, wm_z_value)
...
```{r}
wm_z_all_thr <-
  left_join(wm_z_th.1_wide,
            wm_z_th.12_wide,
            by = "ELS_ID")
wm_z_all_thr <-
  left_join(wm_z_all_thr,
            wm_z_th.14_wide,
            by = "ELS_ID")
wm_z_all_thr <-
  left_join(wm_z_all_thr,
...
### write csv
```{r}
wm_z_aucs_select_fp <-"~/Box/Mooddata_Coordinating/1_Lab_Coordinating/Users/JackieSchwartz/PHIND_RS_and_Suicide/Octo2020Analysis/wm_z_aucs.csv"
write_csv(wm_z_aucs_select,wm_z_aucs_select_fp)
```
